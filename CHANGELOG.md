...
- :bug: Fix HLS proxy streams failing with `403 Invalid or expired signed URL` after 10 minutes of playback. A variant playlist URL is a long
lived handle that a player reloads for the life of a live stream, so it is now signed with a session length token rather than a segment length one
